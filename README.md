...
| `drc_klayout` | `gds_file: string`, `deck_file?: string`, `pdk?: "sky130A"`, `width_um?: number`, `space_um?: number`, `cwd?: string` | KLayout batch `-b -r` | Generated generic width/space smoke deck, or a PDK/custom `.drc` deck. Parses `.lyrdb` to per-rule counts. Sky130 `li.6` LEF-abstract pin hits are classified as `informational` (reported, not dropped, not treated as `clean: false`). Smoke-level, not foundry signoff. |
...
`ghcr.io/zesun33/asic` is the default (anyone can pull). Local builds still work as `localhost/zesun33/asic` via `MCP_GDS_IMAGE`.
