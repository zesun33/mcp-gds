#!/usr/bin/env bash
# verify.sh — 10-gate verification flow for mcp-gds.
#
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      cat <<'EOF'
verify.sh — mcp-gds verification suite
...
run_gate_6() {
  ...
  grep -q "gds_info" README.md || fail 6 "README missing gds_info docs"
...
case "${GATE}" in
  ...
  "")
    ...
    run_gate_6
    echo ""
    echo -e "\033[0;32m=== All Gates Cleared: mcp-gds Verified ===\033[0m"
    ;;
  *)
